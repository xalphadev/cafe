...
export function MenuClient() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background">
      ...
      {/* ── Header ── */}
      <header className="sticky-header bg-white" style={{ boxShadow: "0 1px 0 oklch(0.92 0.025 148)" }}>
    ...
    >
      ...
      <div className="relative w-full overflow-hidden" style={{ aspectRatio: "1/1" }}>
        {product.image ? (
          ...
          <div className="w-full h-full flex items-center justify-center" style={{ background: "oklch(0.91 0.048 148)" }}><UtensilsCrossed className="w-10 h-10 text-gray-300" /></div>
        )}
        ...
        <div className="flex-1">
          ...
          {product.description && (
            <p className="text-[11px] mt-0.5 line-clamp-1"
              style={{ color: "oklch(0.52 0.042 150)" }}>
              {product.description}
            </p>
          )}
          {hasOptions && (
